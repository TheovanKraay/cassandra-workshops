...
In each iteration, the query resumes at the last point changes were read, using paging state. We can see a continuous stream of new changes to the table in the Keyspace. We will see changes to rows that are inserted, or updated. 

> NOTE: Delete operations using change feed in Cassandra API is currently not supported.

The current version of the pull-based change feed model for Cassandra API supports change tracking on either a per logical partition key (primary key) basis, or on the entire container. For large scale implementations, the use of Change Feed should be run in parallel across partitions.

> If this is your first lab and you have not already completed the setup for the lab content see the instructions for [Account Setup](00-account_setup.md) before starting this lab.

## Build A .NET Console App to Generate Data

In order to simulate data flowing into our store, in the form of actions on an e-commerce website, we'll build a simple .NET Console App to generate and add documents to our Cosmos DB CartContainer table.
...
1. For the ``_contactPoint_`` variable, replace the placeholder value with the **CONTACT POINT** value from your Azure Cosmos DB account

    > For example, if your **contact point** is ``https://cosmos-cjg.cassandra.cosmos.azure.com``, your new variable assignment will look like this: ``private static readonly string _endpointUri = "cosmos-cjg.cassandra.cosmos.azure.com;``.

1. For the ``_password`` variable, replace the placeholder value with the **PRIMARY KEY** value from your Azure Cosmos DB account

    > For example, if your **password** is ``elzirrKCnXlacvh1CRAnQdYVbVLspmYHQyYrhx0PltHi8wn5lHVHFnd1Xm3ad5cn4TUcH4U0MSeHsVykkFPHpQ==``, your new variable assignment will look like this: ``private static readonly string _password = "elzirrKCnXlacvh1CRAnQdYVbVLspmYHQyYrhx0PltHi8wn5lHVHFnd1Xm3ad5cn4TUcH4U0MSeHsVykkFPHpQ==";``.

 1. For the ``_username`` variable, replace the placeholder value with the **USERNAME** value from your Azure Cosmos DB account

    > For example, if your **username** is ``cosmos-cjg``, your new variable assignment will look like this: ``private static readonly string _username = "cosmos-cjg";``.

    > Keep the **contact point**, **username** and **password** values recorded, you will use them again later in this lab.
...
7. Expand the **StoreDatabase** then the **CartContainer** and select **Rows**. You should see something like the following screenshot.
...
The two main options for consuming the Cosmos DB change feed are Azure Functions and the Change Feed `TODO` function. We'll start with the Change Feed Processor via a simple console application.
